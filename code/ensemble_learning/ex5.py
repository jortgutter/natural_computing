...
for i in range(baseline_depths):
    for run in range(N_RUNS):
        tree = DecisionTreeClassifier(max_depth=(i+1)*5, random_state=42+i)
...
for i in range(5):
    ...
    plt.plot(range(1,31), np.mean(f_scores[i], axis=0), color=colors[i], linewidth=2, label=f'Forest, depth={i+1}')
for j in range(baseline_depths):
    plt.hlines(y=np.mean(f_scores_baselines[j], axis=0), xmin=1, xmax=max_trees, color='black', linestyles=styles[j], label=f'Single tree, depth={(j+1)*5}')
plt.legend()
